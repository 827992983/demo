...
#ifdef _WIN64
	char szPayloadDll[] = "D:\\code\\EasyHookDemo\\x64\\Release\\messagebox_hook_dll.dll";
#else
	char szPayloadDll[] = "D:\\code\\EasyHookDemo\\Release\\messagebox_hook_dll.dll";
#endif
	int dwPayloadPathLength = strlen(szPayloadDll) + 1;
	TCHAR buffer[1024];
	/* Get the PID of the target we want to inject into */
	DWORD dwTargetPid = GetPidByName(_T("target_app.exe"));
